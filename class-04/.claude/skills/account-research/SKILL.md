...
- Set `confidence` low and say so plainly in `possible_pain_points` or
  `recent_signals` (e.g. "insufficient information found") rather than
  leaving the low number to speak for itself.
- Still populate `evidence` — even a single entry noting what was checked
  and came up empty (e.g. "no public information found beyond the
  company homepage") is more honest than an empty list next to a
  confident-sounding profile.
